...
`start-dflash.sh` is self-contained but different: it requires the **prebuilt** `lmsysorg/sglang:qwen38-27b-dflash2` image locally — no released SGLang tag has DFlash2 support yet (it merged upstream 2026-08-19, after every published tag including the pinned `qwen38-27b`), so the image carries a derived overlay; the builder, `dflash2_nvfp4_head.patch`, and the 5-file overlay are preserved in the `dflash2-build-artifacts` git tag for rebuilding it. First boot then pulls the ~2.7 GB draft if missing.
...
> Whitelisted for tracking: `start.sh`, `start-dspark.sh`, `stop.sh`, `start-dflash.sh`, `bench/`, `README.md`, `CHANGELOG.md`, `.env.sample`, `LICENSE`, `.gitignore`. Experiment scripts and analysis docs stay untracked by design.
...
| `IMAGE` | `lmsysorg/sglang:qwen38-27b-dflash2` (local) | Prebuilt image that must already exist locally (rebuild from the `dflash2-build-artifacts` git tag if missing). Override with `IMAGE=tag` to use another prebuilt tag. |
...
- If a DFlash2 request dies with `DFlash2 selector requires a dense FP16/BF16/FP32 target lm_head` on an NVFP4 target, the image lacks the quantized-head selector support — rebuild it from the `dflash2-build-artifacts` git tag (builder + patch) and run that image
...
├── start-dflash.sh   # DFlash2 engine (port 8888, bf16 or NVFP4 target); needs the prebuilt image (builder kept in the dflash2-build-artifacts git tag); tracked
├── bench/
